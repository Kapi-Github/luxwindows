import { useContext } from "react";
import { MenuContext } from "../Header";

interface Props {
    tab: Tab;
}

const ThirdTierList = ({ tab }: Props) => {
    const { setOpen, setClose, handleListElementClick, openedTabs } =
        useContext(MenuContext);

    return (
        <div className={`absolute top-[12px] left-full flex flex-col`}>
            <div
                className={`bg-slate-100 shadow-[0_0_12px_rgb(127,127,127)] transition-all duration-300 ease-in-out ${
                    openedTabs.second === tab.name
                        ? "z-[100000] pointer-events-auto"
                        : "z-0 pointer-events-none"
                }`}
                style={{
                    height:
                        openedTabs.second === tab.name
                            ? `${tab.subtabs.length * 50}px`
                            : "0px",
                }}
            >
                {tab.subtabs &&
                    tab.subtabs.map((thirdTierTabs) => (
                        <div
                            key={`first-tier-item-${thirdTierTabs.name}`}
                            className={`flex items-center cursor-pointer whitespace-nowrap transition-all duration-300 ease-in-out ${
                                openedTabs.second === tab.name
                                    ? "h-[50px] opacity-100"
                                    : "h-0 opacity-0"
                            }`}
                            onMouseEnter={() => setOpen(thirdTierTabs, "third")}
                            onMouseLeave={() => setClose("third")}
                        >
                            <div
                                className="flex flex-1 h-full items-center gap-[4px] px-[16px] hover:bg-slate-200 transition-all duration-300 ease-in-out"
                                onClick={() =>
                                    handleListElementClick(thirdTierTabs.key)
                                }
                            >
                                <span>{thirdTierTabs.name}</span>
                            </div>
                        </div>
                    ))}
            </div>
        </div>
    );
};

export default ThirdTierList;
